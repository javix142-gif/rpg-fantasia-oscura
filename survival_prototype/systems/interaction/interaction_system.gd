...
static func nearest_resource(host: Node, max_dist: float) -> Dictionary:
	var best: Dictionary = {}
	var nearest: float = max_dist + 1.0
	...
	for y in range(cc.y - 1, cc.y + 2):
		for x in range(cc.x - 1, cc.x + 2):
			...
			for r_variant in chunk["resources"]:
				...
				var d: float = rp.distance_to(host.player_pos)
...
static func nearest_chest(host: Node, max_dist: float) -> int:
	if host.spatial_index == null:
		return -1
	return int(host.spatial_index.nearest_of_type(host.buildings, host.player_pos, max_dist, "cofre"))

static func interaction_label(host: Node) -> String:
	if host.menu_mode != "" or host.placement_type != "":
		return ""
	var chest: int = nearest_chest(host, 46.0)
	if chest >= 0:
		return "ABRIR COFRE"
	var r: Dictionary = nearest_resource(host, 48.0)
	if not r.is_empty():
...
static func interact(host: Node) -> void:
	...
	if chest >= 0:
		...
		return
	var target: Dictionary = nearest_resource(host, 48.0)
	if target.is_empty():
