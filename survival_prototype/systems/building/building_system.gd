...
static func select_build(host: Node, kind: String) -> void:
	if not host.BUILD_RECIPES.has(kind):
		return
	...
	if not host._can_pay(recipe["cost"]):
		...
		return
	host.menu_mode = ""
	host.placement_type = kind
	host.placement_rotation = 0
	host.placement_pos = snap_build_pos(host.player_pos + host.facing * 72.0)
	host._set_major("Mueve el fantasma y confirma", 1.6)
	host._play_sound("ui")
...
static func set_placement_from_screen(host: Node, screen_pos: Vector2) -> void:
	var world := host.player_pos + (screen_pos - host.CENTER)
	var offset := world - host.player_pos
	if offset.length() > host.BUILD_RANGE:
		offset = offset.normalized() * host.BUILD_RANGE
	host.placement_pos = snap_build_pos(host.player_pos + offset)

static func placement_valid(host: Node, pos: Vector2) -> bool:
	if pos.distance_to(host.player_pos) < 34.0 or pos.distance_to(host.player_pos) > host.BUILD_RANGE + 2.0:
		return false
	var test_build := {"type": host.placement_type, "pos": pos, "rot": host.placement_rotation}
	if host._building_blocks_circle(test_build, host.player_pos, host.PLAYER_RADIUS + 2.0):
		return false
	if host.spatial_index != null:
		for b_variant in host.spatial_index.nearby_buildings(host.buildings, pos, 54.0):
...
static func place_build(host: Node) -> void:
	...
	if not placement_valid(host, host.placement_pos):
		host._spawn_floater(host._world_to_screen(host.placement_pos) + Vector2(0, -22), "BLOQUEADO", Color("ef7777"))
		host._play_sound("error")
		return
	...
	var building := {"type": host.placement_type, "pos": host.placement_pos, "rot": host.placement_rotation, "chest": chest_store}
	host.buildings.append(building)
	if host.spatial_index != null:
		host.spatial_index.register_building(host.buildings.size() - 1, building)
	host._mark_dirty("building_placed")
	host._spawn_particles(host.placement_pos, Color("d7c39a"), 16)
	host._set_major("Construido: %s" % String(recipe["name"]), 1.4)
	host._play_sound("build")
	host.placement_type = ""
	host.placement_drag_touch = -1
	host._save_game()

static func cancel_placement(host: Node) -> void:
	host.placement_type = ""
	host.placement_drag_touch = -1
	host.placement_mouse_drag = false
	host._play_sound("ui")
